update_beta_extensive_0 <- function(Y_list,
                                    X_list,
                                    Beta_new,
                                    rho,
                                    Phi = NULL,
                                    ...
                                    verbose = 0)
{
    ...
    while (iter < max.iter & !conv) {
        ...
        for (ii in 1:NN) {
            ...
            yi <- c(t(Y_list[[ii]][, -DD])) # vec for beta update
            XXi <- NULL
            vec_pi <- NULL
            PP <- NULL
            VV <- NULL
            ...
            for (jj in 1:Ti) {
                ...
                XXi <- rbind(XXi, kronecker(diag(1, DD - 1), t(xx)))
            ...
            # build V tilde
            RR <- kronecker(rho^abs(matrix(1:Ti, Ti, Ti, TRUE) - 1:Ti), diag(1, DD - 1))
            eig_VV <- eigen(VV)
            ...
            # get Sd, S
            dtvm <- tryCatch({
                t(XXi) %*% PP %*% solve(V_tilde)
            }, error = function(e)
            {
                t(XXi) %*% PP %*% MASS::ginv(V_tilde)
            })
            sd <- sd + dtvm %*% PP %*% XXi
            ss <- ss + dtvm %*% (yi - vec_pi)
        }
        Beta_curr <- c(t(Beta_curr[-DD, ]))
        upd <- tryCatch({
            solve(as.matrix(sd)) %*% ss
        }, error = function(e)
        {
            MASS::ginv(as.matrix(sd)) %*% ss
        })
        ...
        Beta_new <- rbind(matrix(Beta_new, DD - 1, byrow = TRUE), 0)
        if(jack)
            return(Beta_new)
        ...
        for (ii in 1:NN) {
            ...
            yi <- c(t(Y_list[[ii]][, -DD] / tot_yi)) # de-extensive
            mat_pi <- NULL
            res_mat <- NULL
            for (jj in 1:Ti) {
                # covariates at time jj
                ...
                res_mat <- rbind(res_mat, t(solve(Pi) %*% (yi[((jj - 1) * (DD - 1) + 1):(jj * (DD - 1))] - pi))) # de-extensive modification
